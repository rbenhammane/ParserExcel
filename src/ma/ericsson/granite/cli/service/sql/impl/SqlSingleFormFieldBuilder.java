    ...
    public void setQueries(Form form) {
        ...
        for (Field field : form.getFieldList()) {
            ...
            output.add("INSERT INTO \"EAT\".\"M_FORM_FIELD\"\n" +
                    ...
                    + (field.isReadOnly() ? "Y" : "N") + 
                    "', '*', '*', 'INWI', " + 
                    (field.getxPosition() + FIELD_WIDTH) + ", " + field.getyPosition() + ", "
                    + (field.getWidth() != null ? field.getWidth() : FIELD_WIDTH) + ", " +
                    (field.getHeight() != null ? field.getHeight() : "20") + ");\n");
            output.add(SQL_BLANK_LINE);
            index++;
        }
