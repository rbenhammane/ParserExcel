...
public class SRMSModelGenerator {
	...
	private final static String SOURCE_PATH = "template/src-gen/jpa";
...
//		classeName = StringUtils.capitalize(classeName.toLowerCase());
		...
		annotationSource.setLiteralValue("strategy", "GenerationType.SEQUENCE");
		/*************/
		/*************/

		/** Other fields */
		for (String attrName : mapNameType.keySet()) {
			String attrType = mapNameType.get(attrName);

//			String attributName = Utils.normalize(attrName);
			String attributName = attrName.replace("(", "").replace(")", "");
			attributName = attributName.replace("-", "");
			...
			annotation.setLiteralValue("name", "\"" + attributName + "\"");
...
//		jsonSerialize.setLiteralValue("using", "GenericJsonSerializer.class");
		...
		annotationTable.setStringValue("name", "V_" + classeName);
